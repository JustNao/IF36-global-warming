...
dataSeeLevelRise <- read_csv('./data/See_Level_Rise.csv')
tempColor <- '#ec7c73'
```
...
growth_map <- function(angle = 60, pointSize = 4){
  ggplot() + 
    geom_jitter(data = anomalies, aes(x = lon, y = lat, color = `Temp (°C)`), size = pointSize, alpha = 0.5) +
...
Si l'on prend la station météorologique la plus au Nord du Groenland on constate en effet qu'il y a eu en 2018 un pic de température positif en été. 

```{r, out.width='100%'}
plot <- GreenlandStation %>%
  filter(`MEAN TEMPERATURE (deg C)` > -800, `MEAN TEMPERATURE (deg C)` < 800) %>% # On enlève les relevés sans températures
  filter(`YEAR-MONTH` <= as.Date(as.character("2019-01-01")), `YEAR-MONTH` >= as.Date(as.character("2018-01-01"))) %>%
  group_by(`YEAR-MONTH`) %>%
  summarize(Temp = mean(`MEAN TEMPERATURE (deg C)`)) %>%
    ggplot(aes(x = `YEAR-MONTH`)) +
    geom_line(aes(y = Temp, colour = "Température")) +
    labs(title = "Evolution de la température au nord du Groenland", x = "Date", y = "Température moyenne")
ggplotly(plot)
```
...
dataSeeLevelRiseImproved$`Jason3` <-as.numeric(dataSeeLevelRiseImproved$`Jason3`)
tbl_df(dataSeeLevelRiseImproved)
```

Dans un premier temps on souhaite visualiser l'évolution 

```{r}

plot <- dataSeeLevelRiseImproved %>%
  group_by(`year`) %>%
  summarize(poseidon = mean(`TOPEX/Poseidon`), jason1 = mean(`Jason1`), jason2 = mean(`Jason2`), jasonSeaLevel3 = mean(`Jason3`)) %>%
    ggplot(aes(x = `year`)) +
    # geom_line(aes(y = poseidon, colour = "Poseidon")) +
    # geom_line(aes(y = jason1, colour = "Jason 1")) +
    # geom_line(aes(y = jason2, colour = "Jason 2")) +
    geom_line(aes(y = jasonSeaLevel3, colour = "Jason 3")) + #Pas de données alors qu'il y en a dans le dataset de base
    labs(title = "Evolution de la quantité de précipitation sur les deux côtes",  colour = "Données" )
ggplotly(plot)
