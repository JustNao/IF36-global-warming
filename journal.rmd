...
data %>%
  ...
  summarize(globalTemp = mean(`MEAN TEMPERATURE (deg C)`)) %>%
    ...
    geom_smooth(aes(y = globalTemp), method = 'loess', formula = 'y ~ x', color = "green", se=F) +
    geom_smooth(aes(y = franceTemp$temp), method = 'loess', formula = 'y ~ x', color = "blue", se=F) +
    geom_smooth(aes(y = australiaTemp$temp), method = 'loess', formula = 'y ~ x', color = "red", se=F) +
    ylim(0, 30) + 
